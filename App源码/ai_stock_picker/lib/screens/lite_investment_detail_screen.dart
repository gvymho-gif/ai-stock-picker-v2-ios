...
class LiteInvestmentDetailScreen extends StatefulWidget {
  final LiteInvestmentService service;
  final String portfolioId;

  const LiteInvestmentDetailScreen({
    Key? key,
  ...
  @override
  State<LiteInvestmentDetailScreen> createState() => _LiteInvestmentDetailScreenState();
}

class _LiteInvestmentDetailScreenState extends State<LiteInvestmentDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_portfolio == null) {
      return Scaffold(
        ...
        appBar: AppBar(title: const Text('轻量投资详情')),
        ...
        Row(children: [
          ...
          if (p.newsRating != null)
            Container(
              ...
              decoration: BoxDecoration(
                color: p.newsRating == 'S' ? Colors.red.withOpacity(0.15) : Colors.teal.withOpacity(0.15),
                borderRadius: BorderRadius.circular(6),
              ),
              child: Text('${p.newsRating}级',
                style: TextStyle(
                  color: p.newsRating == 'S' ? Colors.red : Colors.teal,
                  fontSize: 11,
          ...
          if (!isSettled) ...[
            ...
            Container(
              width: 6, height: 6,
              decoration: BoxDecoration(
                color: _refreshing ? colors.liteInvestAccent : (_lastRefreshTime != null ? Colors.green : Colors.grey),
  ...
  Widget _buildPositionCard(VirtualPosition pos, int rank, Map<String, dynamic>? quote, AppColorScheme colors) {
    ...
    if (!isUnfilled) {
      ...
      detailRows.addAll([
        ...
        child: Row(children: [
          Icon(Icons.schedule, size: 14, color: Colors.teal),
        ...
        Row(children: [
          Container(
            ...
            child: Center(child: Text('$rank',
              style: TextStyle(color: colors.liteInvestAccent, fontSize: 12, fontWeight: FontWeight.w800))),
          ),
          const SizedBox(width: AppSpacing.sm),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Color _getStatusColor(PositionStatus status, AppColorScheme colors) {
    switch (status) {
      case PositionStatus.unfilled: return Colors.teal;
