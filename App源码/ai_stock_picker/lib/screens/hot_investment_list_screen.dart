...
class HotInvestmentListScreen extends StatefulWidget {
  final HotInvestmentService service;

  const HotInvestmentListScreen({Key? key, required this.service}) : super(key: key);

  @override
  State<HotInvestmentListScreen> createState() => _HotInvestmentListScreenState();
}

class _HotInvestmentListScreenState extends State<HotInvestmentListScreen> {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          content: Text('云端数据将覆盖本地所有热点投资组合，是否继续？',
  ...
  Widget _buildSettlementHistoryBanner(AppColorScheme colors) {
    ...
    return Padding(
      ...
      child: Material(
        color: archive.isNotEmpty ? colors.hotInvestAccent.withOpacity(0.08) : colors.surface,
          ...
          onTap: () {
            Navigator.push(context, MaterialPageRoute(
              builder: (_) => SettlementHistoryScreen(
                ...
                moduleTitle: '热点投资',
                accentColor: colors.hotInvestAccent,
                accentGradient: colors.hotInvestGradient,
              ),
            )).then((_) => setState(() {}));
          },
          child: Padding(
            ...
            child: Row(children: [
              Icon(Icons.history, size: 16,
                color: archive.isNotEmpty ? colors.hotInvestAccent : colors.textHint),
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              // 名称行
              Row(children: [
                Icon(Icons.whatshot, color: colors.hotInvestAccent, size: 18),
                const SizedBox(width: AppSpacing.sm),
                Expanded(
                ...
                if (portfolio.newsRating != null)
                  Container(
                    ...
                    child: Text(portfolio.newsRating!,
                      style: TextStyle(
                        color: portfolio.newsRating == 'S' ? Colors.red : Colors.orange,
                        fontSize: 12,
  ...
  void _openDetail(HotInvestmentPortfolio portfolio) {
    Navigator.push(
      context,
      _slideRoute(HotInvestmentDetailScreen(
        service: widget.service,
